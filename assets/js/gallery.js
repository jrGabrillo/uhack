...
function(a, b) {
	...
	function e(a, b, f) {
		return this instanceof e ? ("string" == typeof a && (a = document.querySelectorAll(a)), this.elements = d(a), this.options = c({}, this.options), "function" == typeof b ? f = b : c(this.options, b),
			f && this.on("always", f), this.getImages(), h && (this.jqDeferred = new h.Deferred), void setTimeout(function() {
				this.check()
	...
	return e.prototype.addElementBackgroundImages = function(a) {
		...
		this.progressedCount++, this.hasAnyBroken = this.hasAnyBroken || !a.isLoaded, this.emitEvent("progress", [this, a, b]), this.jqDeferred && this.jqDeferred.notify && this.jqDeferred.notify(this, a),
			this.progressedCount == this.images.length && this.complete(), this.options.debug && i && i.log("progress: " + c, a, b)
		...
		return a ? void this.confirm(0 !== this.img.naturalWidth, "naturalWidth") : (this.proxyImage = new Image, this.proxyImage.addEventListener("load", this), this.proxyImage.addEventListener("error",
			this), this.img.addEventListener("load", this), this.img.addEventListener("error", this), void(this.proxyImage.src = this.img.src))
...
function(a, b) {
	...
	var g, h = "undefined" == typeof console ? b : function(a) {
		...
		i = ["paddingLeft", "paddingRight", "paddingTop", "paddingBottom", "marginLeft", "marginRight", "marginTop", "marginBottom", "borderLeftWidth", "borderRightWidth", "borderTopWidth",
			"borderBottomWidth"
		],
		j = i.length,
		k = !1;
...
function(a, b) {
	...
	"function" == typeof define && define.amd ? define("outlayer/outlayer", ["ev-emitter/ev-emitter", "get-size/get-size", "fizzy-ui-utils/utils", "./item"], function(c, d, e, f) {
		return b(a, c, d, e, f)
	}) : "object" == typeof module && module.exports ? module.exports = b(a, require("ev-emitter"), require("get-size"), require("fizzy-ui-utils"), require("./item")) : a.Outlayer = b(a, a.EvEmitter, a
		.getSize, a.fizzyUIUtils, a.Outlayer.Item)
	...
	return d.extend(m, b.prototype), m.option = function(a) {
		...
		if (void 0 !== a) {
			var c = this.size;
			c.isBorderBox && (a += b ? c.paddingLeft + c.paddingRight + c.borderLeftWidth + c.borderRightWidth : c.paddingBottom + c.paddingTop + c.borderTopWidth + c.borderBottomWidth), a = Math.max(a, 0),
		...
		var c = g(f);
		return c.defaults = d.extend({}, f.defaults), d.extend(c.defaults, b), c.compatOptions = d.extend({}, f.compatOptions), c.namespace = a, c.data = f.data, c.Item = g(e), d.htmlInit(c, a), i && i.bridget &&
			i.bridget(a, c), c
	}, f.Item = e, f
}),
function(a, b) {
	"function" == typeof define && define.amd ? define(["outlayer/outlayer", "get-size/get-size"], b) : "object" == typeof module && module.exports ? module.exports = b(require("outlayer"), require(
...
var CanvasImage = function(a) {
	this.canvas = document.createElement("canvas"), this.context = this.canvas.getContext("2d"), document.body.appendChild(this.canvas), this.width = this.canvas.width = a.width, this.height = this.canvas
		.height = a.height, this.context.drawImage(a, 0, 0, this.width, this.height)
};
...
if (ColorThief.prototype.getColor = function(a, b) {
		...
		for (var d, e, f, g, h, i = new CanvasImage(a), j = i.getImageData(), k = j.data, l = i.getPixelCount(), m = [], n = 0; l > n; n += c) d = 4 * n, e = k[d + 0], f = k[d + 1], g = k[d + 2], h = k[d +
			3], h >= 125 && (e > 250 && f > 250 && g > 250 || m.push([e, f, g]));
...
var MMCQ = function() {
	...
	return c.prototype = {
		...
		nearest: function(a) {
			for (var b, c, d, e = this.vboxes, f = 0; f < e.size(); f++) c = Math.sqrt(Math.pow(a[0] - e.peek(f).color[0], 2) + Math.pow(a[1] - e.peek(f).color[1], 2) + Math.pow(a[2] - e.peek(f).color[2],
				2)), (b > c || void 0 === b) && (b = c, d = e.peek(f).color);
			return d
		},
		forcebw: function() {
			var a = this.vboxes;
...
! function(a) {
			...
			return this.each(function(c) {
				function e() {
					if (G) {
						...
						d.off("click.galleryExpand").off("mouseenter.galleryExpand").off("mouseleave.galleryExpand"), g.scrollTop(0), g.attr("style", D.attr("style")), L.css("left", D.find(".gallery-cover").css(
							"left")).removeClass("dh ep"), _(), a("body").css("overflow", ""), g.css("overflowY", ""), d.css({
							transition: "none",
							opacity: .9
						}), M.show(), setTimeout(function() {
							if (a(window).off("resize", B), J.find(".gallery-body").css("display", "none"), P && P.find(U).removeClass("k"), a("nav").removeClass("ed"), a("#placeholder-navbar").fadeOut(b.outDuration,
									"easeInQuad",
									function() {
										a(this).remove()
								...
								var f = {};
								S && Q ? (f.width = n, f.height = o, f.transform = "translate3d(" + p.left + "px, " + V + "px, 0)", f.transition = "transform " + b.outDuration / 1e3 + "s, width " + b.outDuration / 1e3 +
									"s, height " + b.outDuration / 1e3 + "s", f["-webkit-transition"] = "-webkit-transform " + b.outDuration / 1e3 + "s, width " + b.outDuration / 1e3 + "s, height " + b.outDuration / 1e3 +
									"s", $.css({
										background: ""
									})) : (f.transform = "translate3d(" + (p.left - A - V) + "px, 0, 0)", f.transition = "transform " + b.outDuration / 1e3 + "s " + W, f["-webkit-transition"] = "-webkit-transform " + b.outDuration /
									1e3 + "s " + W), L.css(f), M.css({
				...
				var f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F = !0,
					...
					Z = !1;
				g = J.children(".placeholder").first(), g.length || (J.wrapInner(a('<div class="placeholder"></div>')), g = J.children(".placeholder").first()), M.length || (M = a(
					'<div class="gallery-header invisible"></div>'), L.length ? L.after(M) : N.append(M));
				...
				var _ = function() {
					...
					aa = function() {
						origContainerRect = J[0].getBoundingClientRect(), h = origContainerRect.width, i = J.height(), j = M[0].getBoundingClientRect(), k = j.width, l = j.height || 1, n = L.width(), o = L.height(),
							p = L.length ? L[0].getBoundingClientRect() : {
								top: j.top,
								left: 0
							}, q = a(window).scrollTop()
					};
				J.off("click.galleryExpand").on("click.galleryExpand", function(c) {
					if (!F) return !(!a(c.target).is("a") && !a(c.target).closest(".gallery-action").length) && void 0;
					F = !1, window.clearTimeout(C), C = null, aa(), b.dynamicRouting && (window.location.hash = K);
					var d, q, _, ba, ca, da = function() {
						...
						if (Z = Z && ("visible" == c.overflow || "visible" == c.overflowY || "auto" == c.overflow || "auto" == c.overflowY), T = f ? 56 : 64, t = -J[0].getBoundingClientRect().top, u = void 0 ===
							u ? -g[0].getBoundingClientRect().left : u + -g[0].getBoundingClientRect().left, V = f ? 20 : b.contentPadding, v = Math.round(o / 2 + V + T), d = o / 2 - V, q = .7 * X, _ = Y - d - T,
							A = Math.max((X - q) / 2, V), f && (q = X, A = 0), S)
							if (Q) {
					...
					if (a("nav").length && (la = a("nav").css("background-color"), a("nav").addClass("ed")), ha.css({
						...
						ma.onload = function() {
							w.getColor(ma)
						}, ma.crossOrigin = "Anonymous", ma.src = J.find("img").attr("src");
						try {
					...
					if (qa.transform = "", qa.transition = "transform " + b.inDuration / 1e3 + "s", qa["-webkit-transition"] = "-webkit-transform " + b.inDuration / 1e3 + "s", S)
						if (Q) {
							qa.width = r, qa.height = s, qa.transition = "transform " + b.inDuration / 1e3 + "s, width " + b.inDuration / 1e3 + "s, height " + b.inDuration / 1e3 + "s", qa["-webkit-transition"] =
								"-webkit-transform " + b.inDuration / 1e3 + "s, width " + b.inDuration / 1e3 + "s, height " + b.inDuration / 1e3 + "s";
					...
					27 === a.keyCode && H && e()
				}), b.dynamicRouting && d === K && J.trigger("click.galleryExpand")
			})
		},
		open: function() {
	...
	a.fn.galleryExpand = function(c) {
		return b[c] ? b[c].apply(this, Array.prototype.slice.call(arguments, 1)) : "object" != typeof c && c ? void a.error("Method " + c + " does not exist on jQuery.galleryExpand") : b.init.apply(this,
			arguments)
	}
}(jQuery),
function(a) {
